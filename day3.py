...
def add_points(start, instruction, points, steps_taken_dict, steps_taken_counter):
	move_val = int(instruction[1:])
	new_point = None
	if instruction[0] == 'U':
		for x in range(move_val):
			steps_taken_counter += 1
			new_point = (start[0], start[1]+x)
			points.add(new_point)
			
			if new_point not in steps_taken_dict:
				steps_taken_dict[new_point] = steps_taken_counter
		return (start[0], start[1]+move_val), steps_taken_dict, steps_taken_counter

	elif instruction[0] == 'R':
		for x in range(move_val):
			steps_taken_counter += 1
			new_point = (start[0]+x, start[1])
			points.add(new_point)
			
			if new_point not in steps_taken_dict:
				steps_taken_dict[new_point] = steps_taken_counter
		return (start[0]+move_val, start[1]), steps_taken_dict, steps_taken_counter
...
def place_paths(line1, line2):
	...
	for instruction1 in line1:
		start, steps_taken_line1, step_counter1 = add_points(start, instruction1, line1_points, steps_taken_line1, step_counter1)

	# print steps_taken_line1
	# print step_counter1

	start = (0,0)
	for instruction2 in line2:
		start, steps_taken_line2, step_counter2 = add_points(start, instruction2, line2_points, steps_taken_line2, step_counter2)

	return line1_points.intersection(line2_points), steps_taken_line1, steps_taken_line2

paths, l1_steps, line2_steps = place_paths(line1, line2)

def min_steps(intersection_set, line1_steps, line2_steps):
	intersection_set.remove((0,0))
	min_steps = 1000000000000000

	for point in intersection_set:
		total = line1_steps[point] + line2_steps[point]
		# print point, total
		# print "line1", line1_steps[point]
		# print "line2", line2_steps[point]
		if total < min_steps:
			min_steps = total
	return min_steps - 2

print min_steps(paths, l1_steps, line2_steps)


def get_closest(intersection_set):
...
def find_distance(line1, line2):
	paths, line1_steps, line2_steps = place_paths(line1, line2)
	return get_closest(paths, line1_steps, line2_steps)

# print find_distance(line1, line2)
